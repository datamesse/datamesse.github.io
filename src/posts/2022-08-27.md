...
This is my second attempt with Deneb, and this time I've designed a more advanced and fancier zoomable scatterplot. The centre of the axis is Earth, and the data points around it are satellites, whereby their distance from the centre reflects their average orbital distance from Earth, and their circumferential positions relative to the centre reflect the year they were launched.
...
TThe idea to use Sin and Cos for the calculations to map the satellites as a scatterplot is based on Yuli Wang's Tableau Public report. However, in that Tableau report, the calculations additionally and cleverly remove some of the distance scaling based on orbit class in order to closely group the satellites so they are all neat and evenly onscreen. The trade-off from this is that the visual does not accurately reflect outlier satellites whose distances from Earth are tremendously greater than the others.

As at the time of writing, Tableau does not appear to have a way to zoom into an image to retain that scaling, whereas Deneb (i.e. Vega-Lite) does.

For the next step, import Deneb into your report, adding an instance of it to the page, and resize it to the full screen.
